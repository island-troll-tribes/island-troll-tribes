...
function Trig_Bush_Selection_Scout_Actions takes nothing returns nothing
    local unit trollUnit    = getPlayersTroll(GetTriggerPlayer())
    local unit bush         = GetTriggerUnit()
    local integer itemCount = 0
    local integer i         = 0

    if GetUnitTypeId(bush) == UNIT_SCOUTS_BUSH then
...
function InitTrig_Bush_Selection_Scout takes nothing returns nothing
    ...
    call TriggerAddAction( gg_trg_Bush_Selection_Scout, function Trig_Bush_Selection_Scout_Actions )
endfunction
