...
function checkChainReveal takes nothing returns boolean
    return ((GetFilterUnit() != udg_parameterUnit) and (IsUnitEnemy(GetFilterUnit(), GetOwningPlayer(udg_parameterUnit2))) and GetConvertedPlayerId(GetFilterPlayer()) < PLAYER_NEUTRAL_AGGRESSIVE)
endfunction
