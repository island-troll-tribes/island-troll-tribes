...
function QMBaseGloves takes nothing returns boolean
    return ( checkBaseGloves(GetFilterItem()) ) and ( GetWidgetLife(GetFilterItem()) > 0)
endfunction
...
local item i = null
    if((UnitInventorySizeBJ(u) - UnitInventoryCount(u)) >= slotsNeeded) then
        ...
        set i = RandomItemInRectBJ(GetRectFromCircleBJ(GetUnitLoc(u), range),Condition(function QMBaseGloves))
